...
void ShouldDeleteDownloadedManifest() {
    if (!IO::FileExists(checkFilePath2)) {
        ...
        if ((localInstalledVersion == "0.0.0") || (localInstalledVersion == "0.0.1") || (localInstalledVersion == "0.0.2") || (localInstalledVersion == "0.0.3") || (localInstalledVersion == "0.0.4")) {
            ...
            log("YEK, this should only happen if it's your first install, please ignore this if it is :ok:", LogLevel::Error, 15);
            log("(this was added as a failsafe for the like 20 people who installed the plugin before the manifest change)", LogLevel::Error, 16);
            CreateCheckFile2();
        }

    } else {
        log("initCheck file exists in plugin storage, not moving data", LogLevel::Info, 21);
    }
}
